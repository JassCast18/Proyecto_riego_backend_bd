DROP FUNCTION IF EXISTS fn_obtener_dataset_ia_corte(INT,INT,BIGINT);
CREATE OR REPLACE FUNCTION fn_obtener_dataset_ia_corte(p_proyecto_id INT,p_limite INT,p_hasta_decision_id BIGINT)
...
LANGUAGE sql AS $$
 ...
 WHERE f.tb_proyecto_id=p_proyecto_id AND (sh.tipo_componente ILIKE '%higr%' OR sh.tipo_componente ILIKE '%hum%')
   ...
   AND th.fecha_hora<=COALESCE((SELECT d.fecha_hora FROM tb_decision_ia d WHERE d.id=p_hasta_decision_id AND d.tb_proyecto_id=p_proyecto_id),'-infinity'::TIMESTAMP)
