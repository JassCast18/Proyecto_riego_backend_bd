...
export async function calibrateSensorRequest(sensorId,payload){try{return (await api.post(`/control-manual/sensores/${sensorId}/calibracion`,payload)).data?.message}catch(e){throw new Error(message(e,'No fue posible guardar la calibración.'))}}
export async function completeSensorRepairRequest(sensorId,observacion){try{return (await api.patch(`/control-manual/sensores/${sensorId}/reparacion-completada`,{observacion})).data?.message}catch(e){throw new Error(message(e,'No fue posible completar la reparación.'))}}
